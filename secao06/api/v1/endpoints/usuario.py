...
from core.security import gerar_has_senha
...
async def post_usuario(usuario: UsuarioSchemaCreate, db: AsyncSession = Depends(get_session)):
    novo_usuario: UsuarioModel = UsuarioModel(nome=usuario.nome, sobrenome=usuario.sobrenome, email=usuario.email, senha=gerar_has_senha(usuario.senha), eh_admin=usuario.eh_admin)
    async with db as session:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail='Já existe um Usuario com esse email')
...
async def put_usuario(usuario_id: int, usuario: UsuarioSchemaUp, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if usuario_up:
            ...
            if usuario.senha:
                usuario_up.senha = gerar_has_senha(usuario.senha)
